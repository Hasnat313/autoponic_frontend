...
const WeatherScreen = ({navigation, route}) => {
  ...
  if(temp.status == 'loading' ||  humid.status == 'loading' ||  moisture.status == 'loading' ){
    isLoading = true
  }
  else{
    isLoading = false
            ...
            style={{
              ...
              marginVertical: 20,
                ...
                <Icon
                    ...
                    />
            <Text style={{color: '#fff', fontSize: 32, fontWeight: '600' , marginBottom : responsiveHeight(1.4)}}>
              {temperature}°C
            </Text>
          </View>
          <View
            ...
            <TouchableOpacity
              ...
              onPress={() => navigation.navigate('Chart' , {type : route.params.type ? route.params.type  : null})}>
...
{!isLoading ? (
          <FlatList
            ...
            renderItem={({ item }) => {
              ...
              const date = new Date(item.createdAt).toLocaleString(undefined, options);
              let label ;
              let icon_name;
              if(type== 'temperature'){label = "°C"; icon_name = 'temperature'}
              if(type == 'humidity'){label = 'gm/3' ; icon_name = 'humidity'}
              if(type == 'moisture'){label = 'wfv ' ; icon_name = 'moisture'}
